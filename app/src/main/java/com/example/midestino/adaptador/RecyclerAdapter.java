package com.example.midestino.adaptador;
...
import com.example.midestino.R;
...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.RecyclerHolder> {
    ...
    public void onBindViewHolder(@NonNull final RecyclerHolder holder, final int position) {
        ...
        Picasso.get().load(doamin_image+item.getImageResource()).into(holder.imageItem);
    ...
    public void filter(final String strSearch) {
        if (strSearch.length() == 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                items.clear();
                List<ItemList> collect = originalItems.stream().filter(i -> i.getnombre().toLowerCase().contains(strSearch)).collect(Collectors.toList());
